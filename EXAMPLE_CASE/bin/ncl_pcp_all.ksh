...
#export EXE_ROOT=#export EXE_ROOT=/glade/p/ral/jnt/Aerocivil/EXAMPLE_CASE/bin
...
#export FCST_TIME_LIST="00 03 06 09 12 15 18 21 24"
#export ACCUM_TIME=03

# Print run parameters
${ECHO}
${ECHO} "ncl_pcp_all.ksh started at `${DATE}`"
${ECHO}
...
workdir=${MOAD_DATAROOT}/nclprd/pcp${FCST_TIME}
${RM} -rf ${workdir}
${MKDIR} -p ${workdir}
cd ${workdir}
...
if [[ ${MODEL} == "ARW" || ${MODEL} == "NMM" ]]; then
  ${LN} -s ${MOAD_DATAROOT}/postprd/wrfprs_${DOMAIN}_${FCST_TIME}.tm00 infile.grb
  ${ECHO} "${LN} -s ${MOAD_DATAROOT}/postprd/wrfprs_${DOMAIN}_${FCST_TIME}.tm00 infile.grb"
  ${LN} -s ${MOAD_DATAROOT}/metprd/pcp_combine/wrfpcp_${DOMAIN}_${ACCUM_TIME}_${FCST_TIME}.nc infile.nc
  ${ECHO} "${LN} -s ${MOAD_DATAROOT}/metprd/pcp_combine/wrfpcp_${DOMAIN}_${ACCUM_TIME}_${FCST_TIME}.nc infile.nc"
elif [ ${MODEL} == "NMB" ]; then
  ${LN} -s ${MOAD_DATAROOT}/postprd/nmbprs_${DOMAIN}_${FCST_TIME}.tm00 infile.grb
  ${ECHO} "${LN} -s ${MOAD_DATAROOT}/postprd/nmbprs_${DOMAIN}_${FCST_TIME}.tm00 infile.grb"
  ${LN} -s ${MOAD_DATAROOT}/metprd/pcp_combine/nmbpcp_${DOMAIN}_${ACCUM_TIME}_${FCST_TIME}.nc infile.nc
  ${ECHO} "${LN} -s ${MOAD_DATAROOT}/metprd/pcp_combine/nmbpcp_${DOMAIN}_${ACCUM_TIME}_${FCST_TIME}.nc infile.nc"
else
  ${ECHO} "ncl_all crashed! file.grb undefined."
  exit 1
fi

${ECHO} "infile.grb" > arw1_file.txt
${ECHO} "infile.nc" >> arw1_file.txt

set -A ncgms    sfc_acp
set -A pngs     sfc_acp.png
set -A webnames 3hap_sfc
...
${ECHO} "ncl_pcp_all.ksh completed at `${DATE}`"
