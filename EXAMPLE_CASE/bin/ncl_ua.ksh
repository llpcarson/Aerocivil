...
#export FCST_TIME_LIST="03"
#export DOMAIN_LIST="d01 d02"

# Print run parameters
${ECHO}
${ECHO} "ncl_ua.ksh started at `${DATE}`"
${ECHO}
...
if [[ ${MODEL} == "ARW" || ${MODEL} == "NMM" ]]; then
  ...
  ${ECHO} "file.grb" > arw1_file.txt
  ${LN} -s ${MOAD_DATAROOT}/postprd/nmbnat_${DOMAIN}_${FCST_TIME}.tm00 file_nv.grb
  ${ECHO} "${LN} -s ${MOAD_DATAROOT}/postprd/nmbnat_${DOMAIN}_${FCST_TIME}.tm00 file_nv.grb"
  ${ECHO} "file_nv.grb" > arw1_nv_file.txt
else
  ${ECHO} "ncl_ua crashed! file.grb undefined."
  exit 1
fi
...
${ECHO} "ncl_ua.ksh completed at `${DATE}`"
