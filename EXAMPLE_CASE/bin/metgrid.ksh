...
if [ ! "${MODEL}" ]; then
  ${ECHO} "ERROR: \$MODEL is not defined!"
  exit 1
fi
...
${CAT} ${NAMELIST} | ${SED} "s/\(${start}_${date}\)${equal}'${yyyymmdd_hhmmss}', '${yyyymmdd_hhmmss}'/\1 = '${start_yyyymmdd_hhmmss}', '${start_yyyymmdd_hhmmss}', '${start_yyyymmdd_hhmmss}'/" \
                      | ${SED} "s/\(${end}_${date}\)${equal}'${yyyymmdd_hhmmss}', '${yyyymmdd_hhmmss}'/\1 = '${end_yyyymmdd_hhmmss}', '${end_yyyymmdd_hhmmss}', '${end_yyyymmdd_hhmmss}'/"     \
                      > ${NAMELIST}.new
...
# d01 metgrid output must be available for all forecast hours
  fcst=0
  while [ ${fcst} -le ${FCST_LENGTH} ]; do
    time_str=`${DATE} +%Y-%m-%d_%H:%M:%S -d "${START_TIME} ${fcst} hours"`
...
# d02 and d03 metgrid output will only be available for the 00 hour (as we currently run)
  fcst=0
  time_str=`${DATE} +%Y-%m-%d_%H:%M:%S -d "${START_TIME} ${fcst} hours"`
  ...
  if [ ${MAX_DOM} -ge 3 ]; then
      if [ ! -e "${metgrid_prefix}.d03.${time_str}.${metgrid_suffix}" ]; then
	  ${ECHO} "${METGRID} for d03 failed to complete"
	  ${MPIRUN} ${EXIT_CALL} 1
	  exit
      fi
  fi
