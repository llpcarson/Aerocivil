...
${RM} -f Vtable
${LN} -s ${MOAD_DATAHOME}/Vtable.${SOURCE} Vtable
...
${CAT} ${NAMELIST} | ${SED} "s/\(${start}_${date}\)${equal}'${yyyymmdd_hhmmss}'/\1 = '${start_yyyymmdd_hhmmss}'/" \
...
# Create links to the grib files
#if [ ${#gribfiles[*]} -gt ${#id[*]} ]; then
#  ${ECHO} "ERROR: The number of gribfiles exceeds the maximum number of links allowed"
#  exit 1
#fi
#i=0
#while [ ${i} -lt ${#gribfiles[*]} ]; do
#  ${LN} -s ${SOURCE_PATH}/${gribfiles[${i}]} ${workdir}/GRIBFILE.${id[${i}]}
#  (( i= i + 1 ))
#done
